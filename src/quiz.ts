...
let firstTryScore = 0;
let initialQuestionsCount = 24; // Set to official test length
let answered = false;
...
export function startSession(): void {
    ...
    // Limit session to exact test length (24 questions)
    sessionQueue = allWithSM2.slice(0, 24);
    initialQuestionsCount = sessionQueue.length;
    firstTryScore = 0;
    scoreEl.textContent = String(firstTryScore);
...
function render(): void {
    ...
    const progressPct = Math.round(
        ((itemNo - 1) / initialQuestionsCount) * 100,
    );
    ...
    container.innerHTML = `
                ...
                <h2 class="text-lg sm:text-xl font-semibold leading-snug mb-5">${currentQuestion.q}</h2>
                <div class="space-y-2">
                    ${currentDisplayOptions
                        .map(
                            (optObj, i) => `
                        <button data-idx="${i}" class="w-full text-left p-4 border border-line rounded-xl hover:border-accent hover:bg-accent/5 active:bg-accent/10 transition-colors option-btn flex justify-between items-center gap-3">
                ...
                ${isMulti ? `<button id="submit-multi-btn" class="mt-5 w-full bg-accent hover:bg-accent-dark text-white font-medium text-sm py-3 px-4 rounded-xl disabled:opacity-40 disabled:cursor-not-allowed transition-colors" disabled>Check Answers</button>` : ""}
            `;
}
...
function processResult(isCorrect: boolean): void {
    ...
    let qStats = getSM2(questionText);
    ...
    if (isCorrect) {
        ...
        saveSM2(question.q, question.sm2);
...
function checkMulti(): void {
    ...
    selectedOptions.forEach((renderedIdx) => {
        ...
        if ((currentQuestion!.a as number[]).includes(origIdx)) {
            ...
            buttons[renderedIdx].classList.remove(
                "bg-accent/10",
                "border-accent",
            );
        } else {
...
function showResults(): void {
    ...
    restartBtn.textContent = "Start New Mock Test";
}
...
export function initQuiz(): void {
    ...
    container.addEventListener("click", (e) => {
        ...
        const submitBtn = (e.target as HTMLElement).closest(
            "#submit-multi-btn",
        );
        if (submitBtn) {
            checkMulti();
