...
function TrendArrow({ direction }: { direction: "up" | "down" | "flat" }) {
  const d =
    direction === "up"
      ? "M6 18L18 6M18 6H9M18 6V15"
      : direction === "down"
        ? "M6 6L18 18M18 18H9M18 18V9"
        : "M5 12H19";
  return (
...
function tierFor(accuracy: number): { label: string; className: string } {
  if (accuracy >= 90)
    return { label: "Expert", className: "bg-accent text-white" };
  if (accuracy >= 75)
    return { label: "Test Ready", className: "bg-good text-white" };
  if (accuracy >= 50)
    return { label: "Improving", className: "bg-good-soft text-good" };
...
export default function StatsPage() {
  ...
  const friendsByAccuracy = [...friendsList].sort(
  ...
  const halfwayAccuracy = (sessions: typeof history) =>
    sessions.length > 0
      ? (sessions.reduce((sum, h) => sum + h.score, 0) /
          sessions.reduce((sum, h) => sum + h.total, 0)) *
        100
      : 0;
  const mid = Math.floor(history.length / 2);
  const earlierAccuracy = halfwayAccuracy(history.slice(0, mid));
  const laterAccuracy = halfwayAccuracy(history.slice(mid));
  const trendDelta = Math.round(laterAccuracy - earlierAccuracy);
  const TREND_STEADY_THRESHOLD = 3;
  const trend =
    mid === 0
      ? null
      : trendDelta > TREND_STEADY_THRESHOLD
        ? { label: "Improving", className: "text-good", direction: "up" as const }
        : trendDelta < -TREND_STEADY_THRESHOLD
          ? { label: "Declining", className: "text-bad", direction: "down" as const }
          : { label: "Steady", className: "text-muted", direction: "flat" as const };
    ...
    <div className="order-3 flex flex-col gap-6 sm:bg-surface sm:rounded-2xl sm:border sm:border-line sm:shadow-lg sm:shadow-slate-200/60 dark:shadow-none py-2 sm:p-7">
      ...
      <div className="flex flex-col gap-3">
        ...
        <div className="grid grid-cols-2 gap-2">
          <div className="rounded-xl border border-line bg-surface p-5 text-center tabular flex flex-col justify-center gap-1">
            <div className="flex items-center justify-center gap-1.5 text-xl font-semibold text-accent">
              <svg
                className="w-4 h-4"
                fill="none"
                stroke="currentColor"
          ...
          <div className="rounded-xl border border-line bg-surface p-5 text-center tabular flex flex-col justify-center gap-1">
            {trend ? (
              <div
                className={`flex items-center justify-center gap-1 text-xl font-semibold ${trend.className}`}
              >
                <TrendArrow direction={trend.direction} />
                {trend.label}
              </div>
            ) : (
              <div className="text-xl font-semibold text-muted">—</div>
            )}
            <div className="text-[11px] text-muted">
              vs your earlier tests
            </div>
          </div>
        </div>
          ...
          <>
            <div className="grid grid-cols-2 gap-2 tabular">
              <div className="rounded-xl border border-line bg-surface p-3 text-center">
                <div className="text-xl font-semibold">{global.totalTests}</div>
                <div className="text-[11px] text-muted">
              ...
              <div className="rounded-xl border border-line bg-surface p-3 text-center">
                ...
                <div className="text-[11px] text-muted">Average accuracy</div>
              </div>
            </div>
