...
Each question tracks `{ n, ef, i, next, attempts, correct, lastCorrect?, lastSelected? }`. **This data lives only in React state** (`src/progressContext.tsx`'s `ProgressProvider`, a `Record<number, SM2Data>`) — there is no `localStorage` involved. For anonymous users this means progress is lost on refresh (a deliberate tradeoff — see "App-state layer" below); for signed-in users, `AuthSync.tsx` calls `useProgress().refreshFromServer()` on sign-in, which fetches `/api/progress` and merges it into state. `buildSessionQueue()` in `src/quiz/useQuizEngine.ts` sorts the question bank by `next` (overdue first) then by historical accuracy (weakest first) before slicing to session length — this is the prioritization logic that makes practice sessions adaptive.
